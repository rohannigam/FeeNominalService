...
Description: Creates utility functions for audit logging and data management (transaction-related logic removed)
Dependencies: 
...
- (All transaction-related triggers and functions removed)
- All triggers and functions are now created only if they do not already exist
*/
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_surcharge_providers_updated_at'
    ) THEN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_surcharge_provider_configs_updated_at'
    ) THEN
...
-- (All transaction_history, transaction_audit_logs, and transaction-related triggers/functions removed)

-- Function to log audit details (guarded)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE $$
        CREATE FUNCTION fee_nominal.log_audit_details()
        RETURNS TRIGGER AS $$
        DECLARE
            v_audit_log_id UUID;
            v_entity_id UUID;
...
-- Create triggers for audit logging (guarded)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'audit_merchants'
    ) THEN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'audit_api_keys'
    ) THEN
...
-- Create log_audit_event function (guarded)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE $$
        CREATE FUNCTION fee_nominal.log_audit_event(
...
-- Create log_audit_detail function (guarded)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE $$
        CREATE FUNCTION fee_nominal.log_audit_detail(
            p_audit_log_id UUID,
...
-- Create get_audit_logs function (guarded)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE $$
        CREATE FUNCTION fee_nominal.get_audit_logs(
            p_entity_type VARCHAR(50),
            p_entity_id UUID,
